...
[`docs/VALIDATION.md`](VALIDATION.md) says what every gate is checked against. For
the same models in plain terms with worked numbers instead, read
[`docs/HOW_IT_WORKS.md`](HOW_IT_WORKS.md).

| Strand | Object(s) | Section |
|---|---|---|
| Optimal control | $\dot{x} = f(x,u)$, $u \in U(x)$ | [1](#1-optimal-control--the-continuous-dynamics) |
...
| Game theory | $v = \max_x \min_y x^{\mathsf{T}} A y$ | [4](#4-game-theory--sensing-against-counter-sensing) |
| Partial observability | belief $b_t(s)$ given $z_{1:t}$ | [5](#5-partial-observability--belief-and-the-value-of-not-seeing) |
| Combinatorial optimisation | $\max \sum_{ij} \text{payoff}[i][j] x_{ij}$ over an assignment | [6](#6-combinatorial-optimisation--who-shoots-what) |
...
For an airframe the state is $x = (p, \psi, v)$ - position, heading, speed - and the
...
A rate limit is what separates a flight path from a polyline. This addresses the fact that
drones typically cannot turn 90° on the spot and instead flies an arc of radius $r_{\min}$,
arriving late and displaced. That error is exactly what an air defence's engagement window
is made of, so modelling heading as a state with a bounded derivative is load-bearing rather
than decorative.
...
$R = \lVert p - c \rVert$ exactly and gives the lap time $T = \frac{2\pi R}{v}$ in closed
...
`move_cost` is mobility × slope × distance (∞ for impassable); $\text{risk} \in [0,1]$ is a
supplied exposure raster. Because every cost is non-negative there are no negative
cycles, so Dijkstra returns $J^*$ exactly - a label-setting value iteration, not an
...
useful answer than any single "optimal" route. Additive costs were chosen over maximising
survival - $\prod(1 - p_{\text{death}})$ - because the logarithm turns that into the same
...
where $\sigma_m$ is signature in the sensor's modality, $\tau = \exp(-\kappa L)$ the canopy
...
Direct fire scatters isotropically about the aim point with
$\sigma(r) = \delta \cdot r / 1000$ for an angular dispersion $\delta$ mrad. Deflection and
elevation errors are independent, so hitting a $W \times H$ silhouette is a product of two
...
Indirect fire samples a burst $b \sim N(\text{aim}, \sigma^2 I)$ with
$\sigma = \text{CEP} / \sqrt{2 \ln 2}$ - the circular-Gaussian CEP identity - and delivers
the **Carleton** incapacitation kernel $D(\rho) = \exp(-\rho^2 / 2R_L^2)$ at burst-to-target
distance $\rho$. Marginalising the kernel over the burst distribution is a Gaussian
convolving a Gaussian, so the expected damage at aim offset $d$ is exact:
...
Per-unit state $S \in \lbrace \text{Free}, \text{Suppressed}, \text{Pinned} \rbrace$,
stepping up on near-misses at rate $\beta$ and decaying at rate $\mu$:
...
time Pinned → Free is $2/\mu$ - two exponential steps in series. Suppression is a *state*
...
opponent*, presenting the model as a zero-sum matrix game. Blue mixes over
positions $x \in \Delta_m$, Red over routes $y \in \Delta_n$, and von Neumann's minimax theorem
says the game has a value:

$$
v = \max_{x} \min_{y} x^{\mathsf{T}} A y = \min_{y} \max_{x} x^{\mathsf{T}} A y
$$

This is solved by **fictitious play** - each side repeatedly best-responds to the opponent's
...
and the gap $v_{\text{high}} - v_{\text{low}}$ shrinks to zero.
...
position but a **belief** over it: $b_t(s) = P(\text{enemy at } s \mid z_{1:t})$,
maintained by the
two standard steps,
...
A cell a sensor covers well has a low likelihood of producing no detection, so
...
so a jammer raises belief entropy $H(b) = -\sum_s b(s) \log b(s)$ without moving anything.
With no jammers every $g_j = 1$ and the product is exactly 1, so EW-off reduces to the
sensing model bit-for-bit (V40).
...
nothing is, and belief becomes $b'(c) \propto b(c) \big(1 - p(c)\big)$, so
...
A negative result of this kind is only available because the greedy baseline was kept
rather than deleted once the optimal solver worked.
