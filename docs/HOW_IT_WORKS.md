...
`sim_core` never depends on the app or on Bevy. That is what allows ten thousand
...
they return a probability, with no memory and no randomness. `sim/` is the part that
...
**One function is worth reading above the others: `Sim::step_one` in `sim/mod.rs`.** Everything else hangs
off it.
...
point: the numbers are dials to be turned, never values baked into code.
...
what it says produces a finding that can be neither reproduced nor explained.
...
- **`Sim::reset_to_scenario` keeps the terrain and re-rolls only the dice.** That is what
  is wanted for "what happens on *this* map, on average", and what the batch runner uses.
...
probability is a property of the situation **and the choice of tick size**. With a rate,
...
properly. Sitting in woods roughly **doubles** survival time. Sitting in woods *with
foliage between the target and the observer* multiplies it by **137**. Concealment and
transmittance are different things - where a unit stands versus what the sightline passes
