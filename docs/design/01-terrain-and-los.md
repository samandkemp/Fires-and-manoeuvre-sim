...
linear, `hgt(s) = E_a + (E_b − E_a)·s/S`. **Sensing reuses the identical traversal with
...
**Mobility** is exposed as `move_cost(from_cell, to_cell)` on cell **edges**,
not as a baked isotropic raster: slope direction matters, and uphill is penalised harder
than downhill. The constants are placeholder dials, pending a movement stat block.
